...
从存储安全的角度来看，确实风险很高的，但git克隆这个机制完全解除这个风险，我们可以通过clone操作在另外多个的磁盘上建立git克隆，当工作区有新的修改，我们才手动或者自动向版本库做push操作如果使用了网络协议，在其他机器上建立clone,这样就更加安全了（多机备份）。如果在开发团队中，那就每个开发者最少一份本分，这样git版本库就更安全了。
```
...
也无法确认开发者，因为用户信息等可以自己设置，如果工作中开发者数量多，这样出现bug无法确认是谁的提交，容易乱扔锅，或者找不到锅。而工作中，我们会采用代码托管工作，及身份认证
