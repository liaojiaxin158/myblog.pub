...
function cleanAndUpdateBranchRepository()
{
    ...
    if [ ! -d "Code" ]; then
            repo init -u http://szxgit10-rd.huawei.com:8080/HIS/HIS_STB_GIT/HiSTBLinux/manifest.git -b "${branch}" -g synamedia,default
    else
...
function download_changeid(){
    ...
    ChangeNumList=($(ssh -p 29418 phistblinux@szxgit10-rd.huawei.com gerrit query status:open --patch-sets change:"${changeId}"|grep "url: http"|awk -F"/" '{print $NF}'))
    ChangePrjList=($(ssh -p 29418 phistblinux@szxgit10-rd.huawei.com gerrit query status:open --patch-sets change:"${changeId}"|grep "project: "|awk -F"project: " '{print $NF}'))
    if [ "${#ChangeNumList[@]}" != "0" ] && [ "${#ChangeNumList[@]}" == "${#ChangePrjList[@]}" ];then
        pushd "$project_dir"
            i=0
            while [ "$i" -lt "${#ChangeNumList[@]}" ]
            do
                ChangePatch=$(ssh -p 29418 phistblinux@szxgit10-rd.huawei.com gerrit query status:open --patch-sets change:${ChangeNumList["$i"]}|grep "refs/changes/"|awk -F"/" '{print $NF}'|tail -1)
...
function git_cherrypick()
{
    ...
    git fetch http://phistblinux@szxgit10-rd.huawei.com:8080/"${gerrit_project}" "${gerrit_refspec}" && git format-patch -1 FETCH_HEAD  -o "${patch_dir}"
